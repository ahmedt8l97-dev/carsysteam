...
# Load .env from backend folder first, then project root (prefer backend for tokens)
load_dotenv() # Load from CWD (backend/)
env_path = Path(__file__).parent.parent / ".env"
if env_path.exists():
    load_dotenv(dotenv_path=env_path)

app = FastAPI(title="Car Stock - Hybrid Cloud", version="5.0.1")
security = HTTPBearer()
...
def update_product_in_db(product_number: str, updates: dict):
    ...
    if target:
        ...
        valid_fields = ["product_name", "car_name", "model_number", "type", "quantity", "price_iqd", "wholesale_price_iqd", "image", "status", "last_update", "message_id", "product_number"]
...
async def create_product(
    ...
    if product_number and product_number.strip():
        ...
        product_number = None # Let Convex generate it
...
async def update_product(
    ...
    if new_product_number is not None and new_product_number != product_number:
        if new_product_number in cache:
             raise HTTPException(status_code=400, detail="رقم المنتج الجديد موجود بالفعل لقطعة أخرى")
