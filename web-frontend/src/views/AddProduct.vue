...
async function submit() {
  ...
  try {
    ...
    if (file) {
      // Resize to medium quality for fast loading
      try {
        file = await resizeImage(file, 1024, 0.7)
      } catch (err) {
      ...
      <form @submit.prevent="submit" class="form-layout">
        ...
        <div class="form-inputs-grid">
          <div class="input-field">
            ...
            <input v-model="form.product_number" placeholder="مثلاً: A-123 (يترك فارغاً للإنشاء التلقائي)">
          </div>
